...
public class StudentDao {
    ...
    public long addStudent(Student s) {
        ...
        values.put(TableContanst.StudentColumns.FACE_INFO, s.getFaceInfo());
        Log.e("asd", "StudentDao.addStudent"+s.getFaceInfo().toString());
    ...
    public List<Map<String,Object>> getAllStudents() {
        ...
        while(cursor.moveToNext()) {
            ...
            byte[] face = cursor.getBlob(cursor.getColumnIndex(TableContanst.StudentColumns.FACE_INFO));
            map.put(TableContanst.StudentColumns.FACE_INFO, face);
            data.add(map);
        }
        return data;
    }
    ...
    public Student getStudentFromView(View view, long id) {
        ...
        ImageView faceView = (ImageView) view.findViewById(R.id.img);
...
//        byte[] faceInfo = faceView.getText().toString().getBytes();
        //将drawable转为byte[]
        Bitmap bitmap = ImageUtils.drawableToBitmap(faceView.getDrawable() );
        ByteArrayOutputStream os = new ByteArrayOutputStream();
        bitmap.compress(Bitmap.CompressFormat.PNG, 100, os);
        byte[] faceInfo = os.toByteArray();
        Log.d("asd", "StudentDao.getStudentFromView"+faceInfo.toString());
        Student student = new Student(id, name, age, sex, like, phone, data,null, faceInfo);
        return
                student;
    }
}